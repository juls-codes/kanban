...
const AuthView = () => {
  ...
  // Using useState(), we create a state variable with the initial state that is set to 'initForm'. On render, form will have the same structure as 'initForm', with 'email' and 'password' properties initialized as empty strings.
  ...
  return (
    <main className='h-screen text-sm flex flex-col items-center justify-center gap-8 '>
      <section className='text-gray-400 flex flex-col items-center space-y-2'>
        <img src={Logo} alt='Kanban App logo' width={180} height={30}/>
        <h1 className='sr-only text-3xl'>Kanban Board App</h1>
        <p>Effortless task management starts here</p>
      </section>
      ...
      <section className='flex flex-col gap-2 w-[288px]'>
        ...
        <button
          ...
          onKeyDown={(e) => { e.key === 'Enter' && setIsRegistered(!isRegistered)}}
